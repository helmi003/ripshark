...
<table>
    ...
    <?php 
        ...
        if($_SESSION["mail"]==true){
            $id=$_SESSION["mail"];
            $sql1="SELECT * FROM user JOIN session ON user.mail = session.mail where user.mail='$id'";

            $sql2="SELECT * FROM coach JOIN session ON coach.idc = session.idc";

        $sql3="SELECT * FROM club JOIN session ON club.club_mail = session.club_mail";
        ...
        {
            while(($row1 = mysqli_fetch_array($result1))&&($row2 = mysqli_fetch_array($result2))&&($row3 = mysqli_fetch_array($result3)))
            {
                echo "<tr><td>".$row3['club_name']."</td><td>".$row2['coach_name']."</td><td>".$row2['coach_ln']."</td><td>".$row1['date']."</td><td>".$row1['start']."</td><td>".$row1['nbheures']."</td></tr>";
            }
        }else{
            echo "<tr><td colspan=6>No result yet <a href='logout.php'>Log out</a></td></tr>"; 
        }}}else{
            echo "<tr><td colspan=6>No result yet<br>Do you wanna <a href='../../formclient/signin/Formc.php'>Sign in ?</a></td></tr>";
        }
    ?>
</table>
